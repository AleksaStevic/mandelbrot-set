...
export function rgbToXyz(rgb: RGBColor) {
	let r = rgb.r / 255
	let g = rgb.g / 255
	let b = rgb.b / 255
...
export function xyzToLab(xyz: XYZColor) {
	// Observer = 2°, Illuminant = D65
	let x = xyz.x / 95.047
	let y = xyz.y / 100.0
	let z = xyz.z / 108.883
...
export function labToLch(lab: LABColor) {
	const { l, a, b } = lab
...
export function lchToLab(lch: LCHColor) {
	const { l, c, h } = lch
...
export function labToXyz(lab: LABColor) {
	const { l, a, b } = lab
...
export function xyzToRgb(xyz: XYZColor) {
	// Observer = 2°, Illuminant = D65
...
export function lchToRgb(lch: LCHColor): RGBColor {
	const { r, g, b } = xyzToRgb(labToXyz(lchToLab(lch)))

	return { r: Math.trunc(r), g: Math.trunc(g), b: Math.trunc(b) }
}

export function clamp(v: number, range: [number, number]) {
	const [min, max] = range
	if (v < min) return min
	if (v > max) return max
